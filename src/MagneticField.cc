...
// ---------------------------------------------------------------------------------------
{
 // Initializing Magnetic Field
    G4Mag_UsualEqRhs* thisEquation = new G4Mag_UsualEqRhs(this);
    G4MagIntegratorStepper* thisStepper = new G4ClassicalRK4(thisEquation);
    G4ChordFinder* thisChordFinder = new G4ChordFinder(this, 0.01*mm, thisStepper);
    theFieldManager = new G4FieldManager(this, thisChordFinder);
}

// ***************************************************************************************
MagneticField::MagneticField(G4ThreeVector fieldVector):G4UniformMagField(fieldVector)
// ---------------------------------------------------------------------------------------
{
 // Initializing Magnetic Field
    G4Mag_UsualEqRhs* thisEquation = new G4Mag_UsualEqRhs(this);
...
{   
	...
	theFieldManager->SetDetectorField(this);
    } else {
	// If the new field's value is Zero, then it is best to insure 
        //   that it is not used for propagation.
	G4MagneticField* magField = NULL;
	theFieldManager->SetDetectorField(magField);
    }
}
...
// ***************************************************************************************
G4FieldManager*  MagneticField::GetTheFieldManager()
// ---------------------------------------------------------------------------------------
{
