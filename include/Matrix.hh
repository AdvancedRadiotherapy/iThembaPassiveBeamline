...
private:
  Matrix(G4int numberOfVoxelAlongX,G4int numberOfVoxelAlongY,G4int numberOfVoxelAlongZ,G4double massOfVoxel); 

public:

  ~Matrix();
  // Get object instance pointer only
     static Matrix* GetInstance();
  // Make a new & get instance pointer
     static Matrix* GetInstance(G4int nX, G4int nY, G4int nZ, G4double mass);
  // If secondaries data needed 
  ...
  // Store the information of the matrix in a ntuple and in a 1D Histogram
  ...
  // Get a unique index from  a three dimensional one 
     ...
     G4double  GetmassOfVoxel(){ return massOfVoxel;}
...
private:
    ...
    G4double doseDeposited;
    G4double energyDepositedJoule;
